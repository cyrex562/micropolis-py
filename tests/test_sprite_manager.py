...
def test_make_and_destroy_sprite():
    sprite = sprite_manager.new_sprite(context, "t", constants.TRA, 10, 20)
    assert sprite is not None
    assert context.sim.sprites >= 1

    sprite_manager.destroy_sprite(context, sprite)
    assert context.sim.sprites == 0


def test_movement_calls_do_train():
    sprite = sprite_manager.make_new_sprite(context, constants.TRA, 100, 100)
